...
export default function PlanCards() {
  ...
  return (
    ...
    >
      ...
      {/* Plan Kartları */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 max-w-6xl mt-20">
        {plans.map((plan) => {
          ...
          return (
            ...
            >
              <div className="flex flex-col items-center mb-8">
                {plan.icon && (
                  <div className={`text-${plan.color}-500 text-4xl mb-4`}>
                    {plan.icon}
                  </div>
                )}
              ...
              <p
                className={`text-5xl font-bold text-${plan.color}-600 text-center mb-8`}
              >
                {plan.price}
              </p>
              ...
              <ul className="mb-8 space-y-4 text-gray-700 text-lg">
                {plan.features.map((feature, idx) => (
                  <li key={idx} className="flex items-center">
                    <span
                      className={`w-3 h-3 rounded-full bg-${plan.color}-600 inline-block mr-3`}
                    ></span>
                    {feature}
                  </li>
                ))}
